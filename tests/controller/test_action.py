...
def test_action_create(client, session, test_user):
    habit_record = _create_habit(test_user.get("user_id"))
...
@pytest.mark.integration
def test_action_create_unauthorized(client, session, test_user):
    habit_record = _create_habit("123")
    session.add(habit_record)
    session.commit()
...
@pytest.mark.integration
def test_action_create_admin(client, session, test_admin):
    habit_record = _create_habit("123")
    session.add(habit_record)
    session.commit()
